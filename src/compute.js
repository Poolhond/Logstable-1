import { round2, fmtMoney, pad2, fmtClock, formatDurationCompact } from "./utils/format.js";

export { round2, fmtMoney, pad2, fmtClock, formatDurationCompact };
...
export function lineVat(line){ const bucket = line?.bucket || "invoice"; if (bucket === "cash") return 0; return round2(lineAmount(line) * (Number(line.vatRate ?? 0.21)||0)); }
export function bucketTotals(lines, bucket){
  const filtered=(lines||[]).filter(l=>(l.bucket||"invoice")===bucket);
